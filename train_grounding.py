...
def clip_gradient(optimizer, max_norm, norm_type=2):
        for group in optimizer.param_groups:
            torch.nn.utils.clip_grad_norm_(group['params'], max_norm, norm_type=norm_type)
...
def gen_step(optimizer_G, clip_model, real_imgs, text, model, criterion, args):
    ...
    #clip_gradient(optimizer_G, max_norm=0.1, norm_type=2)
...
def main(args=None):
    ...
    model = MultiModel(args=args, clip_model=clip_model, class_names=class_names)
    ...
    # optimizer_G = optim.AdamW(model.parameters(),
    #                       lr=float(args['learning_rate']),
    #                       weight_decay=float(args['WD']),
    #                       betas=(0.9, 0.999))  # 可选：betas 参数默认 (0.9, 0.999)
    ############################################################
    ...
    optimizer_G = optim.AdamW([
        {'params': model.module.backbone.parameters(), 'lr': backbone_lr},  # 主干网络的学习率
        {'params': model.module.text_encoder.parameters(), 'lr': text_encoder_lr},  # 文本编码器的学习率
        {'params': list(model.module.E.parameters()) + 
                list(model.module.D.parameters()) + 
                list(model.module.context_decoder.parameters()) + 
                [model.module.contexts, model.module.gamma], 'lr': other_lr}  # 其他部分的学习率
    ], lr=float(args['learning_rate']), weight_decay=float(args['WD']), betas=(0.9, 0.999))
    ...
    for epoch in range(int(args['epoches'])):
        ...
        bbox, acc = inference_bbox(ds_test, model.eval(), clip_model.eval(), epoch, args)
        f_all.write(str(epoch) + ',' + str('test') + ',' + 'bbox_correctness:' + str(bbox) + ',' + 'hit_correctness:' + str(acc) + '\n')
        f_all.flush()
        if acc > best:
            torch.save(model, args['path_best'])
            best = acc
            f_best.write(str(epoch) + ',' + 'bbox_correctness:' + str(bbox) + ',' + 'hit_correctness:' + str(acc) + '\n')
            f_best.flush()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-lr', '--learning_rate', default=0.00000000000000001, help='learning_rate', required=False)
