...
def inference_wwbl(ds, model, clip_model, text_generator, idx, ss, args, predictions = {}):
    ...
    with torch.no_grad():
        for crops_batch in [blip_crop_imgs[i:i + chunk_size] for i in range(0, len(blip_crop_imgs), chunk_size)]:
            crops_captions.extend(text_generator.generate(torch.cat(crops_batch), sample=False, num_beams=1,
    ...
    for i, inputs in enumerate(pbar):
        ...
        if args['dataset'] == "flicker" or args['task'] == "vg_train" or args['task'] == "coco":
            for sen in meta.keys():
                ...
                bbox_c, hit_c, att_c = calc_correctness(bbox, heatmap.astype(np.float64), size)
                cnt_correct += bbox_c
            ...
            with torch.no_grad():
                ...
                for k, heatmap in enumerate(heatmaps):
                    heatmap = heatmap.mean(dim=0).squeeze().detach().clone().cpu().numpy().astype(np.float64)
                    bbox_c, hit_c, att_c = calc_correctness(bboxes[k], heatmap.astype(np.float64), size)
                    cnt_correct += bbox_c
                    cnt_correct_hit += hit_c
                    att_correct += att_c
                    cnt_overall += 1
...
class_names = ['person', 'bicycle', 'car', 'motorcycle', 'airplane', 'bus', 'train', 'truck', 'boat', 'traffic light', 
                  ...
                  'book', 'clock', 'vase', 'scissors', 'teddy bear', 'hair drier', 'toothbrush']  # coco classname

def main(args=None):
    gpu_num = torch.cuda.device_count()
    model = torch.nn.DataParallel(MultiModel(args=args, class_names=class_names), list(range(gpu_num))).cuda().eval()
    ...
    if args['task'] == 'grounding':
        ...
        model_url = 'yourpath/model_base_capfilt_large.pth'
        image_size = 384
